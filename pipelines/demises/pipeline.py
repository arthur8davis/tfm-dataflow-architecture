"""
Pipeline específico para el schema DEMISES
"""
import sys
import yaml
import argparse
...
class DemisesPipeline:
    """Pipeline para procesar datos de DEMISES"""

    def __init__(self, config_path: str = None, mode: str = "streaming"):
        """
        Inicializa el pipeline de demises
...
def main():
    """Función principal"""
    parser = argparse.ArgumentParser(description='Pipeline DEMISES - COVID-19')
    parser.add_argument(
        '--mode',
        type=str,
    ...
    pipeline = DemisesPipeline(mode=args.mode)
    pipeline.run()
